...
const logout = (req, res) => {
  req.session.destroy((error) => {
    if (error) {
      return res.status(500).json({
        message: "An error occurred, cannot log out",
      });
    }
    res.json({
...
const createOrder = async (req, res) => {
  let { garment, quantity, fabric, colour, deliveryDate, deliveryAddress } =
    req.body;
  if (
    !garment ||
    !quantity ||
...
const forgotPassword = async (req, res) => {
  ...
  const userVerify = await new UserVerification({
    email: email,
    uniqueString,
    createdAt,
    expiresAt,
...
const resetPasswordPost = async (req, res) => {
  ...
  try {
    ...
    await user
      ...
      .then(() => {
        res.status(200).json({
          message: "Password reset successfully, proceedto login",
        });
      })
      .catch((error) => {
